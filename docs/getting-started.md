...
Replacement procedure is documented in the [final support
...
`DEPENDENCIES.txt` files describe their own binary linkage. The v1.0.0
qualification produces exact-source x86_64 and aarch64 archives with a glibc
2.35 gate (observed maximum 2.34), with the native x86_64 qualification
limitation documented in the support matrix.

...
export RESULT_DIR
RESULT_DIR=$(mktemp -d /tmp/gui2tui-p4a-soak-XXXXXX)
CARGO_TARGET_DIR="$TARGET_DIR" CARGO_INCREMENTAL=0 cargo build --bins
dbus-run-session -- bash -euo pipefail -c '
export XDG_RUNTIME_DIR="$RESULT_DIR/runtime" DISPLAY="${DISPLAY_NUMBER:-:104}"
export XDG_SESSION_TYPE=x11 NO_AT_BRIDGE=0 SOAK_SECONDS="${SOAK_SECONDS:-1805}"
...
python3 tests/live/phase4a_soak.py
'
